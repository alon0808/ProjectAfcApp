...
void *GPS_main(void *arg)//int GPS_main(void)
{
	...
	while (1)
	{
		...
		{
			len = 0;
			//PRINT_DEBUGBYS("时间111:", gps_buff, len);
			if (gprmc.pos_state == 'A') {
				len += sprintf(gps_buff + len, "20%02d%02d%02d", gprmc.date % 100, (gprmc.date % 10000) / 100, gprmc.date / 10000);
				// (gprmc.time / 10000 + 8) % 24, (gprmc.time % 10000) / 100, gprmc.time % 100)
				len += sprintf(gps_buff + len, "%02d%02d%02d", (gprmc.time / 10000 + 8) % 24, (gprmc.time % 10000) / 100, gprmc.time % 100);

				len = CharsToBytes(gps_buff, len, gps_buff, len);

				PRINT_DEBUGBYS("时间:", gps_buff, len);
				if (time_invalid(gps_buff) == 1) {//时间有效，校时
					GetDateTime();
					//if(memcmp((unsigned char*)&SysTime.year_h, getbuff, 5) != 0){//分钟不一致，校时
					ret = BCDTime2Long(gps_buff);
