...
SLZR_RESULT CPsamCard_Init(void)
{			
	SLZR_U32 u32Ret;
...
//    SLZR_U8     u8Star = 0;
//    SLZR_U32    u32Len = 0;
    SLZR_U32    i = 0,len,ret=0;
	SLZR_U8     Buffer[100]= {0},status=0;
	SLZR_U8     UserIcNo[6]={0};
	SLZR_U8     receive_buf[1024];
	SLZR_U32	receive_len;
	
	printf("PSAM卡检测\n");									

/*	i = 1;
	u32Ret = m_pIcCardCtrlApi->CMDPSAM_BPS(i, szCardBuf, &u32Len, &u8Star);
	if ( SLZR_SUCCESS == u32Ret )
	{
		printf("bps is OK!\n");
	}
	else
		printf("bps is execute!\n");
...
*/	
	...
	{	
	//	QRCodeInit();
		status=SearchCard(Buffer,&len);
		if(status == 0x08)
		{	
			memcpy(UserIcNo,Buffer,4);
			memcpy(UserIcNo+4,Buffer,2);

			ret = M1CardRead(KEYA, (4*1+1), UserIcNo, receive_buf, &receive_len);
			if(ret == SLZR_SUCCESS)
			{
				printf("M1_SUCC_%d:", receive_len);
				for(i=0;i<receive_len;i++)
					printf("%02x",receive_buf[i]);
				printf("\n");
				printf("\n\n\n\n\n\n\n\n\n\n\nM1 Card read is Success!\n\n\n\n\n\n");
			}
			/*printf("M1卡读卡开始 \n");
			ret = CmdLoadkeySet(UserIcNo,6, &status);
			if((ret == MI_OK) && (status == MI_OK))
			{
				ret = CmdAuthenticationSet(KEYA ,(4*1 + 3), &status);
				if((ret == MI_OK) && (status == MI_OK))
				{
					ret = CmdReadSet((4*1+1), receive_buf, &receive_len, &status);
					if((ret == MI_OK) && (status == MI_OK))
					{
						printf("M1_SUCC_%d:", receive_len);
						for(i=0;i<receive_len;i++)
				...
				//	ret = CmdWriteSet((4*1), receive_buf, receive_len, &status);
			}
			else
			{
				printf("CmdLoadkeySet失败 \n");
		...
		else if(status == 0x20)
		{
			...
			ret = CPUCardRead(receive_buf, &receive_len);
			if(ret == SLZR_SUCCESS)
			{
				printf("M1_SUCC_%d:", receive_len);
				for(i=0;i<receive_len;i++)
		...
		/*	printf("CPU卡读卡开始 \n");			
				...
				SLZR_U8 sw[100], inbuf[50];
				SLZR_U32 inbuf_len = 0;
			...
			if((ret == MI_OK) && (status == MI_OK))
			{
				...
				printf("inbuf_%d:", inbuf_len);
						...
						printf("\n");
					printf("CPU卡读卡信息...");
					ret = CmdApduSet(inbuf, inbuf_len, receive_buf, &receive_len,&status,sw);
					if((ret == MI_OK) && (status == MI_OK))
					{
						printf("M1_SUCC_%d:", receive_len);
						for(i=0;i<receive_len;i++)
...
#ifdef _debug_
	debugstring("PSAM init9999\r\n");
#endif

#ifdef BUS_PRO_JSB
	ii = CmdPSAMbps(1);
#elif defined qPBOC_BUS 
	ii = CmdPSAMbps(0);//使用PBOC
#else
	ii = CmdPSAMbps(0);
