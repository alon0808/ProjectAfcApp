...
static SLZR_U32 UartReadData(SLZR_U8 *pRecvData, SLZR_U32 *pRcvLen, unsigned char *pCmd, unsigned char *pStatus)
{
	SLZR_U32 u32Ret = SLZR_SUCCESS;
	...
	if (pRecvData == NULL || pRcvLen == NULL || pCmd == NULL || pStatus == NULL) {
		return SLZR_FAILURE;
	}

	u32Ret = UartRead(m_u32UartFd, szRcvBuf, prr_lenHead);
	totalLen += u32Ret;
	//SLZR_U8 szBuf[10];	memcpy(szBuf, szRcvBuf, 4);

	if (u32Ret != prr_lenHead)
	{
		printf("[%s] ERROR0----%d\r\n", __FUNCTION__, u32Ret);
		///_ERR_("Uart Read Error!");
		return SLZR_FAILURE;
	}
	if (memcmp(szRcvBuf + prr_Start, "\x7B\x7B\x7B", prr_lenStart) != 0)
	{
		PRINT_ERRORBYS("接收的数据格式头不对:", szRcvBuf, prr_lenHead);
		///_ERR_("Uart Read Data Error!!");
		return SLZR_FAILURE;
	}
	*pCmd = szRcvBuf[prr_Cmd];

	u32RcvLen = GET_INT16S(szRcvBuf + prr_Len);
	if (u32RcvLen > sizeof(szRcvBuf) - prr_lenHeadTail) {
		PRINT_ERR_LOCATION("接收的数据大于1013", "");
		PRINT_ERRORBYS("接收的数据:", szRcvBuf, prr_lenHead);
		return SLZR_FAILURE;
	}

	u32Ret = UartRead(m_u32UartFd, szRcvBuf + prr_lenHead, u32RcvLen + prr_lenTail);
	totalLen += u32Ret;
	if (u32Ret != (u32RcvLen + prr_lenTail))
	{
		printf("[%s] ERROR0----%d\r\n", __FUNCTION__, u32Ret);
		///_ERR_("Uart Read Error!");
		return SLZR_FAILURE;
...
#if 1		
#ifdef _debug_IcApi_
	if (memcmp(szRcvBuf + prr_lenHead, "\x70\x81\xBD\x93", 4) == 0)
	{
		SLZR_U32 i = 0;
		printf("SUCCESS:");
...
#endif
#if 0
	if (memcmp(szRcvBuf + totalLen - prr_lenEnd, "\x7D\x7D\x7D", prr_lenEnd) != 0 || getSignAdd(szRcvBuf, u32RcvLen + prr_lenHead + 1) != szRcvBuf[prr_lenHead + u32RcvLen + 1]) {
		PRINT_ERROR("接收的数据格式尾不对1:%02X, %02X", getSignAdd(szRcvBuf, u32RcvLen + prr_lenHead + 1), szRcvBuf[prr_lenHead + u32RcvLen + 1]);
		PRINT_ERRORBYS("接收的数据格式尾不对:", szRcvBuf, totalLen);
		return SLZR_FAILURE;
	}
#endif
	memcpy(pRecvData, szRcvBuf + prr_lenHead, u32RcvLen);
	*pStatus = szRcvBuf[prr_lenHead + u32RcvLen];
	*pRcvLen = u32RcvLen;
...
SLZR_U32 UartSendAndRecv(SLZR_U8 *pSendData, SLZR_U32 u32SendLen, SLZR_U8 *pRecvData, SLZR_U32 *pRcvLen, SLZR_U8 *u8Status)
{
	...
	if (pSendData == NULL || pRecvData == NULL || pRcvLen == NULL || u8Status == NULL) {
		return SLZR_FAILURE;
	}
	lastCmd = pSendData[prr_Cmd];
#if 0
	PRINT_DEBUG("UartReadData send1%02X,%02X:", lastCmd, *u8Status);
	PRINT_DEBUGBYS("UartReadData send:", pSendData, u32SendLen);
#endif
revagain:
	...
	// ///_DBG_("UartReadData!!");
	u32Ret = UartReadData(pRecvData, &u32Len, &cmd, u8Status);

	if (u32Ret == SLZR_FAILURE) {
		PRINT_ERROR("UartSendAndRecv. if ret= %d\n", u32Ret);
		return SLZR_FAILURE;
	}
#if 0
	PRINT_DEBUG("UartReadData recv1%02X,%02X,statuc=%02X,%d:", cmd, lastCmd, *u8Status, ri);
	PRINT_DEBUGBYS("UartReadData recv:", pRecvData, u32Len);
#endif
	if (lastCmd != cmd) {
		if (ri++ > 3)
			return SLZR_FAILURE;
...
SLZR_U32 CmdPSAMbps(SLZR_U8 BpsFlag)
{
	...
	u32DataLen = 1;
	_DBG_("设置PSAM波特率\n");
	u32Ret = RunCmd(CMD_PSAM_bps, szDataBuf, u32DataLen, szRvBuf, &u32RvLen, &u8Sta);
