...
unsigned char checkNewPrice_new(void)
{
	...
	saveDeviceParaTab(dptm_linePrice, (unsigned char*)&gDeviceParaTab);
...
unsigned char save_csn(void)
{
	...
	for (i = 0; i < len_all; i = i + len)
	{
		...
		case 1://更改商户	bcd
			...
			saveDeviceParaTab(dptm_merchId, buf);//商户号  00009998
			break;
		case 2:	//更改线路 asc
			...
			saveDeviceParaTab(dptm_linePrice, buf);
		...
		case 4:
			...
			saveDeviceParaTab(dptm_devId, gDeviceParaTab.DeviceNo);
			break;
		case 5: //车牌号 asc
...
//把数据清分到msgf中。
	...
	case 0x5001:		//获得消息
		...
		case 2://更改商户	bcd
			...
			{
				saveDeviceParaTab(dptm_merchId, msg_down->msg_buff + 2);//商户号  00009998//				saveCpuInfo(7, msg_down->msg_buff+2);//商户号  00009998
				gGprsinfo.ISOK = 0;	//重签到
			}
		...
		case 3:	//更改线路 asc
			...
			saveDeviceParaTab(dptm_linePrice, getbuff);
			gGprsinfo.ISOK = 0;	//重签到
		...
		case 4: //车牌号 asc
			...
			saveDeviceParaTab(dptm_devId, msg_down->msg_buff);	//设备号
// 			debugstring("车牌号：");
