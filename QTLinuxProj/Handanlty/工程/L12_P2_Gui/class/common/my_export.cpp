...
CMyExport *CMyExport::m_single = NULL;

CMyExport *CMyExport::getInstance()
{
    if(NULL == m_single)
    {
        m_single = new CMyExport(MainWindow::getInstance());
    }
    return m_single;
}

CMyExport::CMyExport(QWidget *parent,char _type) :
    CParentWidget(parent)
{
    m_type = _type;
    this->setFixedSize(540,397);
    this->setObjectName("CMyExport");
    this->setStyleSheet("QDialog#CMyExport{border:1px solid #275ba7;border-radius: 0px;background-color:#03111f;}");//444a59 //ffffff
...
void CMyExport::resizeEvent(QResizeEvent *e)
{
    m_define_screen->reset_widget();
}

void CMyExport::set_content(const QString&_title,const QString&_context,
                                ...
                                const QString &buttonText2)
{
    ...
    {
        int button_x = this->width() - m_pushbutton2->width() - 45;
        int button_y = this->height() - m_pushbutton2->height() - 45;

        m_pushbutton2->move(button_x,button_y);
...
void CMyExport::slot_pushbutton_close()
{
    if(m_timer->isActive())
        m_timer->stop();
    slot_pushbutton2();
...
void CMyExport::slot_pushbutton1()
{
...
void CMyExport::slot_pushbutton2()
{
    if(!close_tip())
        return;
...
void CMyExport::nofity_close()
{
     m_button_type = KEnumButtonNotify;
     hide();
     if(m_timer->isActive())
        m_timer->stop();
}
...
#include "../ltystring.h"
bool CMyExport::close_tip()
{
    bool ret_value = false;

    QString str_content = m_label_content->text();
    str_content = str_content.remove("...");
    CMyMessageBox::getInstance()->set_content(tr("温馨提示"),str_content + tr(",确认中止操作?"),CLtyString::getInstance()->m_confirm,CLtyString::getInstance()->m_cancel);
...
void CMyExport::set_timer(char _timerType)
{
...
void CMyExport::slot_timer_close_window()
{
    if(m_timer_10s <= 0)
...
CMyExport::ButtonType CMyExport::get_result()
{
    return m_button_type;
}

bool CMyExport::keyboard_value(int _value)
{
    bool ret_value = false;
    switch(_value)
    {
